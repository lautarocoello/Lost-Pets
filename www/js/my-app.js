...
$$(document).on('pageInit', function (e) {
    ...
    console.log(tamaño.value)
        ...
        //     document.querySelector("#notas-guardadas").style.display = "block"; //esta linea muestra el div oculto #notas-guardadas
            
            //codigo de recuperacion dada en la documentacion de firebase
            // db.collection("notas").get().then(function(querySnapshot){
            //         querySnapshot.forEach(function(doc){
            
            //             var objeto = doc.data(); // estoy guardando la data que está trayendo en una variable para que sea mas manipulable
            //             // inserto en el DOM el valor id y contenidoNota de la data que trae la variable objeto
            //             document.querySelector("#titulo1").value = doc.id; 
            //             document.querySelector("#nota1").value = objeto.contenidoNota;
...
$$('.infinite-scroll').on('infinite', function () {
  ...
  setTimeout(function () {
    ...
    // '</div>';
    html +=   '<div class="card demo-card-header-pic">'+
        '<div style="background-image:url(https://ponteareasanimal.files.wordpress.com/2011/08/tras-foto.jpg)" class="card-header align-items-flex-end">Perro cargado por el usuario</div>'+
        '<div class="card-content">'+
        '<div class="card-content-inner">'+
         '</div>'+
        '</div>'+
